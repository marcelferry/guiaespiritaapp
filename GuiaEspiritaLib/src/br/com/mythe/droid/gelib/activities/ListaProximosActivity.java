...
public class ListaProximosActivity extends ListaActivity implements
		...
		public void handleMessage(Message msg) {
			switch (msg.what) {
				case NETWOTK_ERROR_WAIT: 
					Toast.makeText(ListaProximosActivity.this, "Erro ao sincronizar conteúdo Web, verifique sua conexão ou tente novamente mais tarde.", Toast.LENGTH_LONG).show();
					break;
			}
		}
	};
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_lista_proximos);		
	...
	private void atualizarCasas(Location origem){
		...
		//} else {
			...
			int[] displayViews = new int[]{
				...
				if(origem != null){			
					Cursor cur = managedQuery(CasasEspiritasProvider.getContentUri(isLite() , Casas.CONTENT ), projection, null, null, " (lat - "+ origem.getLatitude() +") * (lat - "+ origem.getLatitude() +")  + (lng - " + origem.getLongitude() + " ) * (lng - " + origem.getLongitude() + " ) LIMIT 10");		
	...
	private List<CasaVO> carregaCasasEspiritas() throws Exception {
		...
		Location origem = MapUtils.getMyLocation(this);
	...
	class SyncProgressTask extends AsyncTask<Void, Integer, List<CasaVO>> {
	    ...
	    protected void onPostExecute(List<CasaVO> result) {
			...
			setListAdapter(new CasasAdapter(ListaProximosActivity.this, R.layout.lista_casas_item, casas));
	    }
	}
